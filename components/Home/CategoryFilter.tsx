      ...
      {["All", "shonen", "shoujo", "seinen", "josei"].map((category) => (
        <li
          ...
          className={`relative w-[80px] sm:w-[100px] text-[10px] sm:text-[15px] text-center py-1.5 font-bold mb-2 cursor-pointer group ${
            activeCategory === category ? "text-[#d54b87]" : "text-[#333]"
          } mr-4 md:mx-4`}
          onClick={() => onCategorySelect(category)}
        >
          <span className="absolute inset-0 w-full h-full transition duration-300 ease-out transform translate-x-1.5 translate-y-1.5 bg-red-300 group-hover:translate-x-0 group-hover:translate-y-0"></span>
